...
export async function generateToken(config: LiveKitConfig): Promise<string> {
  ...
  if (config.useTokenServer && config.tokenServerUrl) {
    try {
      ...
      throw new Error(`Failed to fetch token from server: ${err instanceof Error ? err.message : 'Unknown error'}`);
    }
  }

  // Validate required fields for client-side generation
  if (!config.apiKey || !config.apiSecret) {
    throw new Error('API Key and Secret are required for client-side token generation');
  }
