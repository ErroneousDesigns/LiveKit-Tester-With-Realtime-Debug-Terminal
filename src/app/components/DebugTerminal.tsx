...
export function DebugTerminal({
  ...
  const [lastNetworkQuality, setLastNetworkQuality] =
    useState<string>("");
  ...
  useEffect(() => {
    ...
    addLog(
      "info",
      `Monitoring: ${participantMode === "streamer" ? "Outbound (Publishing)" : "Inbound (Receiving)"} streams`,
    );
    ...
    console.log = (...args) => {
      ...
      if (message.includes("[Chat]")) {
        ...
        message.includes("[Room Settings]") ||
  ...
  useEffect(() => {
    ...
    addLog(
      "connection",
      `Connection state: ${stateNames[connectionState]}`,
    );
  }, [connectionState]);
  ...
  useEffect(() => {
    addLog(
      "participant",
      `Participants count: ${participants.length} (You + ${participants.length} others)`,
    );
    participants.forEach((p) => {
      const tracks =
        p.videoTrackPublications.size +
        p.audioTrackPublications.size;
      addLog(
        "participant",
        `→ ${p.identity} | Tracks: ${tracks} | Speaking: ${p.isSpeaking ? "Yes" : "No"}`,
      );
    });
  }, [participants.length]);
  ...
  useEffect(() => {
    ...
    const logRoomStats = () => {
      ...
      addLog(
        "info",
        `Room name: ${room.name} | SID: ${room.sid}`,
      );
    };
    ...
    const measurePing = async () => {
      ...
      try {
        ...
        if (Math.abs(ping - lastPing) > 50 && lastPing > 0) {
          addLog(
            "connection",
            `Ping changed: ${lastPing}ms → ${ping}ms`,
          );
        }
  ...
  useEffect(() => {
    ...
    const updateStreamStats = async () => {
      ...
      if (participantMode === "viewer") {
        ...
        if (remoteParticipants.length > 0) {
          videoTrack = Array.from(
            remoteParticipants[0].videoTrackPublications.values(),
          )[0];
        }
      } else {
      ...
      if (videoTrack?.track) {
        try {
          const stats =
            await videoTrack.track.getRTCStatsReport();
          let totalBitrate = 0;
          let packetLoss = 0;
          let jitter = 0;
          let fps = 0;
          ...
          if (
            ...
            addLog(
              "track",
              `Resolution changed: ${resolution} @ ${fps}fps`,
            );
            lastResolution = resolution;
          }

          if (
            Math.abs(totalBitrate - lastBitrate) > 500 &&
            totalBitrate > 0
          ) {
          ...
          if (jitter > 50) {
            addLog(
              "connection",
              `High jitter detected: ${jitter}ms`,
            );
          }
        } catch (err) {
          console.error(
            "[Stats] Failed to get stream stats:",
            err,
          );
        }
      }
    ...
    addLog(
      "track",
      `Starting stream statistics monitoring (${participantMode} mode)...`,
    );
    ...
    const summaryInterval = setInterval(() => {
      ...
      if (stats.bitrate > 0 || stats.fps > 0) {
        addLog(
          "info",
          `Stats Summary | Ping: ${stats.ping}ms | Bits: ${stats.bitrate}kb | FPS: ${stats.fps} | Loss: ${stats.packetLoss}%`,
        );
      }
    }, 60000);
  ...
  useEffect(() => {
    ...
    const handleDataReceived = (
      ...
      try {
        ...
        addLog(
          "chat",
          `[${participant?.identity || "Unknown"}]: ${data.message || JSON.stringify(data)}`,
        );
      } catch (err) {
    ...
    const handleTrackSubscribed = (
      ...
      addLog(
        "track",
        `Track subscribed: ${publication.kind} from ${participant.identity}`,
      );
    };
    ...
    const handleTrackUnsubscribed = (
      ...
      addLog(
        "track",
        `Track unsubscribed: ${publication.kind} from ${participant.identity}`,
      );
    };
  ...
  const getNetworkQuality = (): {
    ...
    if (
      streamStats.ping < 50 &&
      streamStats.packetLoss < 1 &&
      streamStats.jitter < 20
    ) {
      return {
        level: "EXCELLENT",
        color: "#00ff00",
        description: "Optimal streaming conditions",
      };
    } else if (
      streamStats.ping < 80 &&
      streamStats.packetLoss < 2 &&
      streamStats.jitter < 30
    ) {
      return {
        level: "GOOD",
        color: "#00ff00",
        description: "Good streaming quality",
      };
    } else if (
      streamStats.ping < 150 &&
      streamStats.packetLoss < 5
    ) {
      return {
        level: "FAIR",
        color: "#ffaa00",
      ...
      return {
        level: "POOR",
        color: "#ff0000",
        description: "Network issues detected",
      };
    }
  };
  ...
  useEffect(() => {
    ...
    if (
      quality.level !== lastNetworkQuality &&
      lastNetworkQuality !== ""
    ) {
      addLog(
        "connection",
        `Network quality changed: ${lastNetworkQuality} → ${quality.level}`,
      );
    }
    ...
    return (
      ...
      >
        ...
        >
          ...
          <div
            className="px-3 py-2 grid grid-cols-3 gap-2 text-xs font-mono"
  ...
  return (
    ...
    >
      ...
      {/* Filters */}
      <div
        className="flex gap-1 px-2 py-2 border-b flex-wrap"
        style={{
          borderColor: "#333",
          ...
          <div className="flex items-center gap-2">
            ...
            >
              <div
                ...
                style={{
                  backgroundColor: getNetworkQuality().color,
                }}
              />
              <span
                className="text-xs font-mono font-semibold"
                style={{ color: getNetworkQuality().color }}
              >
        ...
        <div className="grid grid-cols-3 gap-2 text-xs font-mono">
          ...
          <div className="flex items-baseline gap-1">
            ...
            <span
              ...
              style={{
                color:
                  streamStats.fps < 20 && streamStats.fps > 0
                    ? "#ffaa00"
                    : "#00ff00",
              }}
            >
